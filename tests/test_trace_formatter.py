...
from isp_trace_parser import trace_formatter, directory_structure_formatter
...
def test_trace_formatter():
    ...
    formatted_data = formatted_data.with_columns([
        pl.col("Datetime").dt.offset_by('-1s').dt.year().cast(pl.Int64).alias("Year"),
        pl.col("Datetime").dt.offset_by('-1s').dt.month().cast(pl.Int64).alias("Month"),
    ...
    formatted_data = formatted_data.with_columns([
        (pl.col("m") // 30).alias('Period')
    ])
    formatted_data = formatted_data.with_columns([
        pl.when(pl.col('Period') == 0).then(pl.lit(48)).otherwise(pl.col('Period')).alias('Period')
    ])
    formatted_data = formatted_data.with_columns([
        pl.col("Period").map_elements(lambda x: f"{x:02}").alias("Period")
