...
from create_end_to_end_test_data import create_solar_csvs, create_demand_csvs, create_wind_csvs
from isp_trace_parser.directory_structure_formatter import restructure_wind_directory, restructure_demand_directory
...
def test_solar_directory_restructure(tmp_path):
    ...
    project_mapping_yaml = (Path(__file__).parent.parent /
                            Path('src/name_mapping/solar_project_mapping.yaml'))
    ...
    combos = itertools.product(config.reference_years, range(config.start, config.end), config.half_years,
                               config.solar_projects)
    for ry, y, half_year, project in combos:
        project = project_name_mapping[project].replace(' ', '_')
        parquet_file = Path(f"RefYear{ry}/Project/{project}/RefYear{ry}_{project}_FFP_HalfYear{y}-"
                        f"{half_year}.parquet")
        assert (new_dir / parquet_file).is_file()

    combos = itertools.product(config.reference_years, range(config.start, config.end), config.half_years,
                               config.areas, config.area_techs)
    for ry, y, half_year, area, tech in combos:
        parquet_file = Path(f"RefYear{ry}/Area/{area}/{tech}/RefYear{ry}_{area}_{tech}_HalfYear{y}-"
                        f"{half_year}.parquet")
        assert (new_dir / parquet_file).is_file()
...
def test_wind_directory_restructure(tmp_path):
    ...
    project_mapping_yaml = (Path(__file__).parent.parent /
                            Path('src/name_mapping/wind_project_mapping.yaml'))
    with open(project_mapping_yaml) as f:
        project_name_mapping = yaml.safe_load(f)
    ...
    for ry, y, half_year, project in combos:
        ...
        parquet_file = Path(f"RefYear{ry}/Project/{project}/RefYear{ry}_{project}_HalfYear{y}-"
                        f"{half_year}.parquet")
        assert (new_dir / parquet_file).is_file()

    combos = itertools.product(config.reference_years, range(config.start, config.end), config.half_years,
                               config.areas, config.area_wind_resources)
    for ry, y, half_year, area, resource in combos:
        parquet_file = Path(f"RefYear{ry}/Area/{area}/{resource}/RefYear{ry}_{area}_{resource}_HalfYear{y}-"
                        f"{half_year}.parquet")
        assert (new_dir / parquet_file).is_file()
...
def test_demand_directory_restructure(tmp_path):
    ...
    demand_scenario_mapping_yaml = (Path(__file__).parent.parent /
                            Path('src/name_mapping/demand_scenario_mapping.yaml'))
    with open(demand_scenario_mapping_yaml) as f:
        demand_scenario_mapping = yaml.safe_load(f)

    combos = itertools.product(config.reference_years, range(config.start, config.end), config.half_years,
                                    config.sub_regions, config.poe, config.demand_type, config.scenarios)
    for ry, y, half_year, area, poe, demand_type, scenario in combos:
        scenario = demand_scenario_mapping[scenario].replace(' ', '_')
        parquet_file = Path(f"{scenario}/RefYear{ry}/{area}/{poe}/{demand_type}/"
