...
@pytest.mark.parametrize("valid_input", [
    ...
    {"name": ["A"], "file_type": ["area"], "technology": ["SAT"], "reference_year": [2011]},
])
...
@pytest.mark.parametrize("valid_input", [
    ...
    {"reference_year": [2011, 2012]},
    {"name": ["A"], "file_type": ["area"], "resource_quality": ["WH"], "reference_year": [2011]},
])
...
@pytest.mark.parametrize("valid_input", [
    ...
    {"reference_year": [2011, 2012]},
    {"subregion": ["CNSW"], "scenario": ["Step Change"], "poe": ["POE50"], "demand_type": ["OPSO_MODELLING"], "reference_year": [2011]},
])
...
@pytest.mark.parametrize("invalid_input,expected_error", [
    ({"scenario": ["invalid"]}, "Input should be 'Step Change', 'Progressive Change' or 'Green Energy Exports'"),
    ({"poe": ["invalid"]}, "Input should be 'POE50' or 'POE10'"),
    ({"demand_type": ["invalid"]}, "Input should be 'OPSO_MODELLING', 'OPSO_MODELLING_PVLITE' or 'PV_TOT'"),
    ({"reference_year": ["invalid"]}, "Input should be a valid integer"),
...
@pytest.mark.parametrize("invalid_input", [
    ...
    {"input_directory": "valid_path", "parsed_directory": "valid_path", "use_concurrency": "not_a_bool"},
    {"input_directory": "valid_path", "parsed_directory": "valid_path", "filters": "not_a_filter"},
])
...
def test_construct_reference_year_mapping_validation_valid():
    result = construct_reference_year_mapping(start_year=2030, end_year=2035, reference_years=[2011, 2013, 2018])
