...
def get_archive_trace(trace_type: str ="solar", 
    ...
    url = urljoin(BASE_URL, f"archive/traces/{trace_type}/{version}/isp_{trace_type}_traces_r{refyear}.zip")
    with requests.get(url, stream=True) as response:
        with open(filepath, 'wb') as file:
            for chunk in response.iter_content(chunk_size=1048576):
                file.write(chunk)

def unzip_trace_archive(zipfilepath: str,
                        destination_dir: str
                        ):
...
def unzip_and_restructure(zipfilepath: str,
...
def get_unzip_and_resturcture(trace_type: str ="solar", 
                              version: str = "isp_2024",
                              refyear: int = 2018,
