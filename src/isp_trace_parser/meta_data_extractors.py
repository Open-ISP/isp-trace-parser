...
def extract_solar_trace_meta_data(filename):
    ...
    pattern1 = re.compile(r"^(?P<name>[A-Za-z0-9_\-]+)_(?P<technology>[A-Z]+)_RefYear(?P<year>\d{4})\.csv$")

    # Case 2: Match filenames that have a rez, a name, and tech, followed by RefYear
    pattern2 = re.compile(r"^[A-Z]+_(?P<name>[A-Z0-9]+)_[A-Za-z0-9_\-]+_(?P<technology>[A-Z]+)_RefYear(?P<year>\d{4})\.csv$")
...
def extract_wind_trace_meta_data(filename):
    ...
    pattern1 = re.compile(r"^(?P<name>.*)_RefYear(?P<year>\d{4})\.csv$")

    # Case 2: Match filenames that have a resource type and a name followed by RefYear
    pattern2 = re.compile(r"^(?P<name>[A-Z0-9]+)_(?P<resource_type>W[A-Z]+)_[A-Za-z_\-]+_RefYear(?P<year>\d{4})\.csv$")
