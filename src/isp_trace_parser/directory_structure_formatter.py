...
def write_new_demand_filepath(meta_data):
    ...
    return (
        f"{scenario}/RefYear{m['year']}/{area}/{m['poe']}/{m['descriptor']}/"
...
def save_half_year_chunk_of_trace(
    chunk, file_meta_data, half_year, new_directory, write_new_filepath
):
...
def get_meta_data_that_matches_trace_names(trace_names, all_old_file_meta_data):
    return {
        f: meta_data
        for f, meta_data in all_old_file_meta_data.items()
        if meta_data["name"] in trace_names
    }


def get_unique_reference_years_in_meta_data(meta_data_for_trace_files):
    return list(
        set(meta_data["year"] for meta_data in meta_data_for_trace_files.values())
    )
...
def get_meta_data_that_matches_reference_year(year, meta_data_for_trace_files):
    return {
        f: meta_data
        for f, meta_data in meta_data_for_trace_files.items()
        if meta_data["year"] == year
    }


def get_unique_techs_in_meta_data(meta_data_for_trace_files):
    return list(
        set(meta_data["technology"] for meta_data in meta_data_for_trace_files.values())
    )
...
def get_unique_resource_types_in_meta_data(meta_data_for_trace_files):
    return list(
        set(
            meta_data["resource_type"]
            for meta_data in meta_data_for_trace_files.values()
        )
    )
...
def get_meta_data_that_matches_resource_type(resource_type, meta_data_for_trace_files):
    return {
        f: meta_data
        for f, meta_data in meta_data_for_trace_files.items()
        if meta_data["resource_type"] == resource_type
    }
...
def restructure_solar_files(
    save_name,
    original_trace_names,
    all_old_file_meta_data,
    new_directory,
    write_new_filepath,
):
    meta_data_for_trace_files = get_meta_data_that_matches_trace_names(
        original_trace_names, all_old_file_meta_data
    )
    reference_years = get_unique_reference_years_in_meta_data(meta_data_for_trace_files)
    for year in reference_years:
        files_for_year = get_meta_data_that_matches_reference_year(
            year, meta_data_for_trace_files
        )
        techs = get_unique_techs_in_meta_data(files_for_year)
        for tech in techs:
            files_for_tech = get_meta_data_that_matches_tech(tech, files_for_year)
            example_meta_data = get_example_meta_data_for_writing_save_name(
                files_for_tech
            )
            example_meta_data = overwrite_meta_data_trace_name_with_save_name(
                example_meta_data, save_name
            )
            process_and_save_files(
                files_for_tech, example_meta_data, write_new_filepath, new_directory
            )
...
def restructure_wind_area_files(
    ...
    for year in reference_years:
        ...
        for resource_type in resource_types:
            files_for_resource_type = get_meta_data_that_matches_resource_type(
...
def restructure_wind_project_files(
    ...
    for year in reference_years:
        ...
        process_and_save_files(
            files_for_year, example_meta_data, write_new_filepath, new_directory
        )
...
def restructure_solar_directory(old_directory, new_directory, use_concurrency):
    ...
    with open(
        Path(__file__).parent.parent / Path("name_mapping/solar_project_mapping.yaml"),
        "r",
    ) as f:
        project_name_mapping = yaml.safe_load(f)
    with open(
        Path(__file__).parent.parent / Path("name_mapping/solar_area_mapping.yaml"), "r"
    ) as f:
    ...
    partial_func = functools.partial(
        ...
        all_old_file_meta_data=file_meta_data,
        new_directory=new_directory,
        write_new_filepath=write_new_solar_filepath,
    )
...
def restructure_wind_directory(old_directory, new_directory, use_concurrency=True):
    ...
    with open(
        Path(__file__).parent.parent / Path("name_mapping/wind_project_mapping.yaml"),
        "r",
    ) as f:
    ...
    with open(
        Path(__file__).parent.parent / Path("name_mapping/wind_area_mapping.yaml"), "r"
    ) as f:
    ...
    area_partial_func = functools.partial(
        ...
        all_old_file_meta_data=file_meta_data,
        new_directory=new_directory,
        write_new_filepath=write_new_wind_filepath,
    )
    ...
    if use_concurrency:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            results = executor.map(
                area_partial_func, area_save_names, area_old_trace_names
            )
            # Iterate through results to raise any errors that occurred.
            for result in results:
                result
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            results = executor.map(
                project_partial_func, project_save_names, project_old_trace_names
            )
            # Iterate through results to raise any errors that occurred.
            for result in results:
                result
        ...
        for save_name, old_trace_name in zip(
            project_save_names, project_old_trace_names
        ):
...
def restructure_demand_file(file, demand_scenario_mapping, new_directory):
    ...
    file_meta_data["scenario"] = get_save_scenario_for_demand_trace(
        file_meta_data, demand_scenario_mapping
    )
    ...
    for half_year, chunk in trace.group_by("HY"):
        save_half_year_chunk_of_trace(
            chunk, file_meta_data, half_year, new_directory, write_new_demand_filepath
        )
...
def restructure_demand_directory(old_directory, new_directory, use_concurrency):
    ...
    with open(
        ...
        / Path("name_mapping/demand_scenario_mapping.yaml"),
        "r",
    ) as f:
